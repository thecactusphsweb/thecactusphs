...
async function fetchArticleBodyHtml(issueSlug, articleId) {
  const data = await api(`/api/article-body?issueSlug=${encodeURIComponent(issueSlug)}&articleId=${encodeURIComponent(articleId)}`);
  return data.html || "";
}
...
editIssueSelect.addEventListener("change", async () => { if (editIssueSelect.value) await refreshArticlesForIssue(editIssueSelect.value); });
...
issueForm.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    const payload = {
      title: fd.get("title"), slug: fd.get("slug"), dateLabel: fd.get("dateLabel"),
...
articleForm.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    const payload = {
      issueSlug: fd.get("issueSlug"), title: fd.get("title"), subtitle: fd.get("subtitle"), author: fd.get("author"),
      date: fd.get("date"), category: fd.get("category"), type: fd.get("type"), tags,
      frontPageSlot: slot, featuredMain: slot === "main", sponsored: false,
      imageCaption: fd.get("imageCaption"), citationsText: fd.get("citationsText"), bodyText: fd.get("bodyText")
    };
...
loadArticleForm.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    editPreviewController.setImageUrl(article.imageUrl ? `/${issueSlug}/${article.slug}/${article.heroFilename || article.imageUrl}` : "");
...
editArticleForm.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    const tags = String(fd.get("tags") || "").split(",").map((x) => x.trim()).filter(Boolean);
    const payload = {
      originalIssueSlug: fd.get("originalIssueSlug"), issueSlug: fd.get("issueSlug"), articleId: Number(fd.get("articleId")),
      title: fd.get("title"), subtitle: fd.get("subtitle"), author: fd.get("author"), date: fd.get("date"),
      category: fd.get("category"), type: fd.get("type"), tags,
      frontPageSlot: fd.get("frontPageSlot"), featuredMain: fd.get("frontPageSlot") === "main", sponsored: false,
      imageCaption: fd.get("imageCaption"), citationsText: fd.get("citationsText"), bodyText: fd.get("bodyText")
    };
    if (hero instanceof File && hero.size) {
      payload.heroBase64 = await fileToBase64(hero);
